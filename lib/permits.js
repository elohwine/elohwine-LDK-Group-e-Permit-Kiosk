...
export async function getSites() {
  if (typeof window === 'undefined') return [];
  try {
    const cached = JSON.parse(localStorage.getItem('sites') || '[]');
    if (Array.isArray(cached) && cached.length) return cached;
  } catch {}
  // Dummy sites list; later can be fetched from API and cached
  const dummy = [
    { id: 'SITE01', name: 'Central Car Park' },
    { id: 'SITE02', name: 'Riverside Lot' },
    { id: 'SITE03', name: 'Airport P4' },
  ];
  try { localStorage.setItem('sites', JSON.stringify(dummy)); } catch {}
  return dummy;
}
